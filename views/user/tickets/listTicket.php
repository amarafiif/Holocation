...
<body class="bg-gray-100">
    ...
    ?>
    <div class="bg-blue-400 px-24 py-4">
        <h1 class="font-semibold text-white py-5 text-3xl">Purchases List</h1>
    </div>
    ...
    if (isset($_SESSION['id'])) {
        $id_user = $_SESSION['id'];
        $sql = "SELECT ticket.id, ticket.ticket_code, destination.name, ticket.price_total FROM ticket INNER JOIN destination ON ticket.id_destination = destination.id  WHERE id_users = $id_user";
    }
    ...
    ?>
            <div class="container mx-auto mt-10 px-14">
                <div class="bg-white border-2 border-gray-200 rounded-lg">
                    <div class="border">
                        <div class="bg-blue-400 w-full p-5 rounded-t-lg">
                            <div>
                                <p class="font-medium text-white uppercase">No. Ticket <?= $row['ticket_code'] ?></p>
                            </div>
                        </div>
                    </div>
                    <a href="">
                        <div class="flex flex-col-2 bg-white w-full p-5 justify-between">
                            <div class=" ">
                                <h1 class="font-medium text-xl"><?= $row['name'] ?></h1>
                            </div>
                            <div>
                                <h1 class="font-medium text-xl">Rp <?= $row['price_total'] ?></h1>
                            </div>
                        </div>
                    </a>
                    <div class="flex flex-col-3 w-full p-5 justify-between">
                        <div class="font-medium text-gray-700">
                            <p class="bg-green-400 px-10 rounded-2xl text-gray-50 font-semibold">Pembelian berhasil</p>
                        </div>
                        <div class="flex space-x-5 font-medium text-blue">
